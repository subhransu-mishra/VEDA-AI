...
export default function About() {
  ...
  return (
    <motion.section id="about" className="relative min-h-[100svh] overflow-hidden">
      <video
        autoPlay
        muted
        loop
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
          <div>
            <div className="text-xs font-medium text-white/70 sm:text-sm">
              {tr("about.breadcrumb", "Home / About Us")}
            </div>

            <h2 className="mt-4 max-w-xl text-4xl font-semibold tracking-[-0.06em] text-white sm:text-5xl lg:text-6xl">
              {tr("about.title", "About Us")}
            </h2>
          </div>

          <div className="lg:pt-16">
            <p className="max-w-md text-sm leading-7 text-white/78 sm:text-base">
              {tr("about.intro1", "VedaAI is built to make the first step into healthcare feel more understandable, guided, and less overwhelming for every patient.")}
            </p>

            <p className="mt-5 max-w-md text-sm leading-7 text-white/72 sm:text-base">
              {tr("about.intro2", "We bring symptom understanding, health record clarity, and smarter care direction into one simple experience so people can move toward the right support with more confidence and less delay.")}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div>
            <h3 className="max-w-xl text-3xl font-semibold leading-tight tracking-[-0.05em] text-white sm:text-4xl lg:text-5xl">
              {tr("about.belief", "We believe that healthcare guidance should feel simple, human, and clear.")}
            </h3>
          </div>

          <div className="lg:justify-self-end">
            <div className="max-w-md text-sm leading-7 text-white/70 sm:text-base">
              {tr("about.outro", "VedaAI helps reduce confusion before treatment begins by giving people a clearer sense of what they are feeling, what matters in their records, and what step should come next.")}
            </div>
          </div>
        </div>
      </div>
