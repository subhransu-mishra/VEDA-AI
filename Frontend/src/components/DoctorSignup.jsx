...
import { createSession, upsertDoctorCache } from "../utils/authStorage";
...
export default function DoctorSignup({ onSignup }) {
  ...
  const validate = () => {
    ...
    if (!phoneRegex.test(form.phone.trim()))
      next.phone = "Enter 10 digit phone number.";
    ...
    if (!form.hospitalName.trim())
      next.hospitalName = "Enter hospital/clinic name.";
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      clinicName: form.hospitalName.trim(),
      city: form.city.trim(),
    };
    ...
    try {
      const response = await authApi.doctorSignup(payload);
      ...
      toast.error(error.message || "Doctor signup failed");
    } finally {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-linear-to-br from-slate-50 via-cyan-50 to-white px-4 py-10 sm:px-6 lg:px-8">
      <motion.div
        animate={{ y: [0, -20, 0], x: [0, 14, 0] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        className="pointer-events-none absolute -left-20 -top-10 h-72 w-72 rounded-full bg-cyan-200/40 blur-3xl"
      />
      <motion.div
        animate={{ y: [0, 25, 0], x: [0, -18, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        className="pointer-events-none absolute -right-20 top-[20%] h-80 w-80 rounded-full bg-sky-200/40 blur-3xl"
      />
      <motion.div
        animate={{ y: [0, -18, 0], x: [0, 10, 0] }}
        transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }}
        className="pointer-events-none absolute -bottom-20 left-[22%] h-72 w-72 rounded-full bg-blue-100/60 blur-3xl"
      />
      ...
      <section className="mx-auto grid max-w-7xl items-start gap-10 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        >
          ...
          >
            Create your doctor account, submit your professional details, and
          ...
          >
            ...
            <motion.div variants={itemVariants}>
              ...
              {errors.licenseNumber && (
                <p className={errorClass}>{errors.licenseNumber}</p>
              )}
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              ...
              />
              {errors.hospitalName && (
                <p className={errorClass}>{errors.hospitalName}</p>
              )}
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              ...
              {errors.experienceYears && (
                <p className={errorClass}>{errors.experienceYears}</p>
              )}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="sm:col-span-2">
              ...
              {errors.clinicAddress && (
                <p className={errorClass}>{errors.clinicAddress}</p>
              )}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="sm:col-span-2 pt-1">
              <label className="flex items-start gap-3 rounded-3xl border border-slate-200/80 bg-slate-50/70 px-4 py-4 text-sm text-slate-600 shadow-sm">
                ...
                />
                <span>
                  I confirm these details are correct and consent to
                  verification.
                </span>
              </label>
            ...
            >
              <motion.button
                ...
                className="rounded-full bg-linear-to-r from-cyan-500 via-sky-500 to-blue-500 px-6 py-4 text-sm font-semibold text-white shadow-[0_15px_35px_rgba(14,165,233,0.28)] transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-70"
              >
