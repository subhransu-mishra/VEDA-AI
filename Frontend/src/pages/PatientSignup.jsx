...
export default function PatientSignup({ onSignup }) {
  ...
  const validate = () => {
    ...
    if (!phoneRegex.test(form.phone.trim()))
      next.phone = "10-digit number required.";
    if (!passwordRegex.test(form.password))
      next.password = "8+ chars, 1 uppercase, 1 number.";
    ...
    if (!Number.isFinite(height) || height < 50 || height > 300)
      next.height = "Valid height (cm) required.";
    const weight = Number(form.weight);
    if (!Number.isFinite(weight) || weight < 2 || weight > 500)
      next.weight = "Valid weight (kg) required.";
    ...
    if (form.emergencyName.trim().length < 3)
      next.emergencyName = "Contact name required.";
    if (!phoneRegex.test(form.emergencyPhone.trim()))
      next.emergencyPhone = "10-digit number required.";
    if (form.primaryConcern.trim().length < 8)
      next.primaryConcern = "Please provide more detail.";
  ...
  return (
    <main className="relative min-h-screen bg-[#F5F5F7] px-6 py-20 selection:bg-slate-300 sm:px-12 lg:px-24">
      ...
      <div className="relative z-10 mx-auto w-full max-w-3xl">
        ...
        >
          <p className="text-sm font-medium tracking-widest text-slate-500 uppercase mb-2">
            Patient Intake
          </p>
          <h1 className="text-3xl font-light text-slate-900 sm:text-4xl tracking-tight">
            Tell us about{" "}
            <span className="italic text-slate-500 hover:text-slate-900 transition-colors duration-500">
              yourself.
            </span>
          </h1>
        </motion.div>
        ...
        >
          ...
          <motion.div
            variants={itemVariants}
            className="grid grid-cols-1 gap-10 sm:grid-cols-4"
          >
            <LineInput
              label="Age"
              name="age"
          ...
          <motion.div
            variants={itemVariants}
            className="mt-8 border-t border-slate-200 pt-8 grid grid-cols-1 gap-10 sm:grid-cols-2"
          >
            <LineInput
          ...
          >
            ...
            <div className="flex items-center justify-between border-t border-slate-200 pt-8">
              <Link
                to="/signup"
                className="text-sm font-semibold tracking-widest text-slate-400 uppercase transition-colors hover:text-slate-900"
              >
                Cancel
              </Link>
              <button
                ...
                className="group relative overflow-hidden rounded-full bg-slate-900 px-8 py-3 text-sm font-semibold tracking-widest text-white uppercase transition-all hover:bg-black active:scale-95 disabled:cursor-not-allowed disabled:opacity-70"
              >
